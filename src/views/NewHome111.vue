<template>
  <div class="newhome">
    ...
    <!-- 图片文字 -->
    <div class="edit_box">
      <div class="edit_item" v-for="item in edit" :key="item.id">
        <img :src="item.picUrl" />
        <div>{{ item.name }}</div>
        <span class="u-earp">{{ parseInt(item.playCount / 10000) }}万</span>
      </div>
    </div>
    <!-- 最新音乐 -->
    <div class="title">
    ...
    <div class="newset_box">
      <div class="newset_item" v-for="item in newset" :key="item.id">
        <!-- 左侧歌曲名和歌手 -->
    ...
    <audio autoplay ref="audio" :src="`https://music.163.com/song/media/outer/url?id=${this.url}.mp3`"></audio>
    <!-- 尾部 -->
    <!-- <div class="footer">
      <div class="logo">
        <svg
      ...
      <p class="copyright">网易公司版权所有©1997-2022   杭州乐读科技有限公司运营</p>
    </div> -->
  </div>
</template>
  ...
  methods: {
    ...
    songs() {
      ...
      this.axios({
        ...
        // 处理数据
        let a = Math.floor(Math.random() * (result.data.result.length - 5));
        console.log(a);
        this.edit = result.data.result.slice(a, a + 6);
    ...
    clickPlay(e) {
      ...
      // 歌曲id给到url
      this.url = e.id
      // 获取对象里面的isPlay 取反 !isPlay true => false, false => true
      e.isPlay = !e.isPlay
      // console.log(e);
      // 获取页面上的audio标签
  ...
  .edit_box {
    ...
    .edit_item {
      ...
      > div {
        ...
        // 溢出隐藏
        overflow: hidden;
        // 多行文本的情况下,用省略号隐藏溢出范围的文本
        text-overflow: ellipsis;
        // 用来控制块级元素显示文本的行数 2表示最多显示的行数
        -webkit-line-clamp: 2;
        // 将对象作为弹性伸缩盒子模型来显示
        display: -webkit-box;
      ...
      .newset_left {
        ...
        .song_auth {
          ...
          .song_left {
            ...
            background: url("../assets/index_icon_2x.png") no-repeat 0 2px;
